...
export default {
  ...
  methods: {
    ...
    loginhand(e) {
      ...
      this.$router.options.routes[0].children[0].hidden = false;
      this.$router.options.routes[0].children[9].hidden = false;
    ...
    logouthand() {
      ...
      this.$router.options.routes[0].children[0].hidden = true;
      ...
      this.$router.options.routes[0].children[9].hidden = true;
      this.$message({
        message: "正在退出...",
        type: "success"
      });
    ...
    getuserlist(username, page) {
      console.log("Get all user name" + username + page);
        ...
        .then(response => {
          ...
          if (response.statusCode === "0") {
            this.postinfo = {
              ...
              postID: id,
              page: page,
              clickNum: response.clickNum,
              responseNum: response.responseNum,
              maxpage: response.maxpage
            };
    ...
    topclick() {
      console.log('你打算看最多点击');
      var url = this.urlprefix + "/find/findtopclickpost.php";
      this.$http
        .post(url, {})
        .then(response => {
          response = response.body;
          console.log(response);
          if (response.statusCode === "0") {
            this.mostclick = response.result;
            // then I decide to do nothing xixixi, for I'm lazy
          } else if (response.statusCode === "1") {
            alert("查询失败");
          }
    ...
    topreply() {
      console.log('你打算看最多回复');
      var url = this.urlprefix + "/find/findtopresponsepost.php";
      this.$http
        .post(url, {})
        .then(response => {
          response = response.body;
          if (response.statusCode === "0") {
            this.mostreply = response.result;
          } else if (response.statusCode === "1") {
            alert("查询失败");
          }
        })
